...
def motion(frame,prev_frame):
	if prev_frame is None:
		return 0
	prev_frame = cv2.resize(prev_frame, (176*5, 144*5)) #frame_size
	prev_frame = cv2.cvtColor(prev_frame, cv2.COLOR_BGR2GRAY)
	prev_frame = cv2.GaussianBlur(prev_frame, (21, 21), 0)



	frame = cv2.resize(frame, (176*5,144*5))#frame_size
	...
	while frame_number_entry != "done":
		...
		if frame_num > -1 and frame_num > prev_frame_num:
			...
			if cnt % 1 ==0:
				...
				blob = cv2.dnn.blobFromImage(frame,0.007843, (frame_size, frame_size), 127.5)				
				net.setInput(blob)
