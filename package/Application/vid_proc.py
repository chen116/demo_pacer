	...
	while frame_number_entry != "done":
		...
		if frame_num > -1 and frame_num > prev_frame_num:
			...
			if detect_car == 1:
				...
				frame_size = heavy_workload_frame_size#light_workload_frame_size
			frame = imutils.resize(frame, width=frame_size)
			(startX, startY, endX, endY)=(0,0,0,0) 
			blob = cv2.dnn.blobFromImage(frame,0.007843, (int(frame_size), int(144*frame_size/176)), 300)#127.5)	
